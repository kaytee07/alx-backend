...
+ [x] 6. **Create the Job creator**
  ...
  + If you execute multiple time this file, you will see the `JOB ID` increasing - it means you are storing new nodemon] to restart at any time, enter `rs`
...
+ [x] 5. **Node Redis client publisher and subscriber**
  + In a file named [`5-subscriber.js`](5-subscriber.js), create a redis client:
    + On connect, it should log the message `Redis client connected to the server`.
    + On error, it should log the message `Redis client not connected to the serverjob to process…
...
+ [x] 9. **Track progress and errors with Kue: Create the Job processor**
  ...
  + **Requirements:**
    ...
    + Executing the jobs list should log to the console the following:
      ...
      Sending notification to 4153118782, with message: cation job #41 0% complete
...
+ [x] 13. **Can I have a seat?**
  + **Redis**
    + Create a Redis client:
      ...
      + CreateThis is the code 4321 to verify your account
      ...
      + And in the same time in terminal 1:
        ...
        Notifi a function `getCurrentAvailableSeats`, it will return the current number of available seats (by using `promisify` for Redis).
